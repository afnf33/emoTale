...
KcBERT는 네이버 뉴스 댓글을 수집하여 Pretrain 된 한국어 BERT 모델로, 다른 BERT 모델들과 달리 정제되지 않은 데이터를 사용하여 신조어나 오탈자 등 인터넷 상의 텍스트에 나타나는 특징이 잘 반영되어 있다. 이번 프로젝트에서는 트위터 트윗을 학습데이터로 선택하였기 때문에 KcBERT가 더 어울릴 것으로 예상되어 공식적인 프로젝트 기간이 지난 뒤 추가시도를 진행하고 있다.  

> 현재 업로드 되어있는 코드의 경우, CUDA error(`device-side assert triggered`)가 발생하여 수정 중이다.
